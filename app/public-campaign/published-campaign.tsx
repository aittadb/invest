      ...
      <main>
            ...
            <div className="content-width aggregate-layout">
              ...
              <div className="aggregate-total">
                ...
                {publicAggregate.oversubscription ? (
                  <p>{aggregateProgress(publicAggregate.oversubscription, publicAggregate.currency)}</p>
                ) : null}
              </div>
            </div>
          </section>
            ...
            <div className="participation-grid">
              {campaign.participation.paths.map((path, index) => (
                <article id={`${path.kind}-interest`} key={path.kind}>
                  ...
                  {campaign.status === "open" ? (
                    <a className="text-link" href={participant ? PARTICIPANT_HOME_PATH : signInPath}>
              ...
              <div className="product-intro">
                ...
                <div className="product-links">
                  {campaign.product.links.map((link, index) => (
                    <a className={index === 0 ? "button button--dark" : "text-link"} href={link.href} key={`${link.label}-${link.href}`} rel={link.rel.join(" ")}>
                      {link.label}
                    </a>
                  ))}
                </div>
        ...
        <section className="risk-band" id="risks" aria-labelledby="risks-title">
          <div className="content-width risk-layout">
            ...
            <ul>{campaign.risks.items.map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
        </section>
        ...
        <section className="closing-band" aria-labelledby="closing-title">
          <div className="content-width closing-inner">
            ...
            {campaign.status === "open" ? (
              <a className="button button--primary" href={participant ? PARTICIPANT_HOME_PATH : signInPath}>
      ...
      <footer>
        <div className="content-width footer-inner">
          <div className="brand brand--footer">
            {campaign.brandMarkUrl ? <img alt="" aria-hidden="true" height="28" src={campaign.brandMarkUrl} width="28" /> : null}
            <span>{campaign.name}</span>
          </div>
          <p>{campaign.footer.tagline}</p>
          <div>
            {campaign.footer.links.map((link) => (
              <a href={link.href} key={`${link.label}-${link.href}`} rel={link.rel.join(" ")}>
                {link.label}
              </a>
            ))}
          </div>
...
function aggregateProgress(progress: PublicOversubscriptionDisplayData, currency: string): string {
