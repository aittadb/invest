...
Each summary contains only `review_id`, `status`, `primary_contribution_area_id`, `updated_at`, and `revision`. When the independently composed detail capability is present, each item has one `item` link to `/owner/founder-applications/{review-id}` and HTML renders the same transition. Applicant subjects, internal application IDs, expertise, notes, profile links, field history, storage keys, and credentials do not enter the collection document, HTML, links, or errors. A collection composed without detail advertises no unavailable transition.
...
The continuation value is the bounded-storage protocol's opaque cursor, not a subject, record key, offset derived by this application, or client-selected identity. AittaDB binds it to the credential namespace, collection, and page parameters and guarantees that cursor page state contains no principal or physical-key detail. The application performs one finite list per HTTP request, rejects no-progress or empty continuation pages, verifies each bounded current application, current field payload, and dedicated review lookup, and can continue a prior cursor after a Worker restart. A current application with an absent, crossed, or corrupt lookup fails closed instead of advertising a broken or misbound detail transition.

`GET /owner/founder-applications/{review-id}` resolves exactly one opaque current application without a collection scan. The established review ID remains the one-way identity of the trusted private subject and internal application ID; it is not the current, history, or field storage key. Its digest intentionally addresses one dedicated private lookup record, whose verified coordinates are used server-side to derive the distinct current key. It accepts no query fields. A malformed identifier returns generic `400`, an absent lookup returns generic `404`, and a crossed lookup, missing current record, corrupt ancestry, or unavailable persistence returns generic `503`; rejected input and backend detail are never reflected. A valid response includes the current founder fields and complete bounded transition history, but no applicant subject, storage key, credential, or owner identity. Pre-index records require an explicit backend-only verified backfill; this GET never creates storage state.

The detail document always links to itself and the owner workspace. It links to the collection only when that independently composed capability is available; detail-only composition omits the unavailable transition from both JSON and HTML. HTML renders the same current state, private fields, professional profile URLs, history, and available navigation as version `0.1` JSON. Profile URLs are validated before persistence; HTML escapes the same URL in link text and attributes, applies `rel="noreferrer"`, and the response sends `Referrer-Policy: no-referrer`. Both representations use the same configured-owner authorization and verified persistent projection, are private and non-cacheable, and remain stable after Worker reconstruction. Anonymous callers receive `401` and authenticated non-owners receive generic `404` before any founder-application detail read.
...
An active item may expose `edit-investment-interest` with `PATCH` when the current package acknowledgment is valid and `withdraw-investment-interest` with `DELETE`. A withdrawn item may expose `reactivate-investment-interest` with `POST` when renewed acknowledgment and deployment policy permit it. Its separate `/participant/investment-interests/{id}/withdrawal-replay` link exposes only `retry-investment-interest-withdrawal` with `DELETE`, carrying the immutable terminal withdrawal operation and prior revision. A rejected item exposes no participant mutation action.
...
When an investment-interest JSON representation advertises at least one mutation action, its response carries a validated participant-bound CSRF proof in the header named by `MUTATION_CSRF_HEADER`. This applies to initial discovery and to the current resource returned after create, edit, withdrawal, or reactivation. A JSON client echoes that value in the same named request header. The proof and header name are not fields in the hypermedia document; responses with no mutation action omit the proof header. HTML forms obtain the same validated proof and submit it only through their hidden CSRF transport field. If a valid proof cannot be issued, the route fails closed instead of advertising unusable actions. The terminal replay subresource requires a distinct one-use exact-replay proof. A fixed-length digest of the canonical replay command binds the participant, canonical replay path, `DELETE`, indication, operation, prior and result revisions, and explicit confirmation without making valid maximum-sized identities or operations exceed the proof boundary. It rejects ordinary action proofs, changed commands, other paths or methods, foreign participants, and reused proofs before another durable claim; an exact retry returns only the already committed withdrawal after response loss or Worker restart.
...
`GET /owner/aggregate-reconciliation` is the configured owner's private comparison of the stored investment-interest aggregate with a fresh calculation from persisted contribution projections. HTML and version `0.1` hypermedia JSON expose the same campaign revision, stored revision, amount, currency, private contributing count, calculated summary, owner link, and current correction capability. Anonymous callers receive authentication guidance, while authenticated non-owners receive the generic not-found surface before aggregate storage is read.

Only a mismatch backed by `atomic-aggregate-audit` persistence whose stored revision is below `Number.MAX_SAFE_INTEGER` exposes `apply-calculated-aggregate`. Its eight body fields bind a server-issued operation ID, the exact advertised campaign revision, and explicit confirmation to every stored and calculated preview value. Every server-issued field is marked `presentation: hidden` in the shared action contract and rendered as hidden HTML; only the explicit confirmation remains a control. JSON accepts exactly those eight fields; native HTML accepts those eight plus its CSRF transport field. Both exact field sets are checked before durable proof claim. New operation IDs and ordinary proofs are issued only when that correction is available, so unrelated matching, post-commit, unavailable-consistency, and maximum-revision states cannot fail merely because issuance is unavailable.

The aggregate record committed by a successful correction also carries one closed, bounded copy of that exact command. While the corrected aggregate remains the matching current state, HTML and JSON expose only `retry-recorded-aggregate-correction` with the same fields and no newly issued operation ID. Before advertising it, the configured-owner capability verifies the immutable operation receipt and audit actor; owner rotation therefore omits the prior owner's command, replay action, and proof. Its proof stores only an encrypted digest binding the configured owner, route, `POST`, operation ID, campaign and aggregate revisions, confirmation, amounts, and counts. After response loss or Worker restart, an exact retry reaches the immutable receipt; changed, foreign, unrelated-method, and reused proofs fail before another durable claim or correction transaction. A later aggregate write removes the terminal command and therefore the retry action.

The hosted Worker passes the complete persisted current campaign revision to a repository bound to the trusted configured-owner subject. For new work, the submitted advertised revision must equal that runtime revision, and the correction compare-and-sets the aggregate revision while committing the corrected snapshot and terminal command, immutable retry receipt, allowlisted `reconciled` audit event, and exact unchanged campaign-revision evidence in one AittaDB transaction. A preview from revision 1 cannot correct under revision 2, and a campaign change during the transaction returns `412` with none of the correction records changed. The returned transaction envelope must be a closed object with one boolean replay marker and four dense, exactly ordered records matching the aggregate, receipt, audit, and campaign-check mutations. Calculation reads at most 1,000 contribution records through 20 pages and rejects oversized, malformed, repeated, empty-continuation, or endlessly advancing pagination. A successful POST builds its returned matching state from the committed result without a second calculation. Receipt lookup precedes the current-revision check: an exact delayed retry decodes and cross-checks its immutable committed currency instead of applying the current campaign currency, so it can recover its original result after any later campaign configuration revision. Terminal reconstruction likewise uses the aggregate's verified immutable currency while new work remains bound to the current revision and amount configuration. One bounded post-transaction receipt-and-audit verification recovers the immutable winner of overlapping exact requests even when their server timestamps differ. Changed reuse conflicts, new stale work fails, and storage failure changes neither aggregate nor audit. Responses remain private and non-cacheable and never expose credentials, storage keys, indication identities, or participant fields.
