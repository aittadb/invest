...
Investment-interest reads and writes are separate injected capabilities. Reads require only participant-owned `get` and `listOwned` operations. Every write requires `AtomicParticipantInvestmentInterestMutationPort` with the explicit `atomic-indication-aggregate-audit` guarantee: the new indication revision, aggregate projection, allowlisted audit transition, and idempotent retry result are committed together or none are changed. Hosted create, edit, and reactivate transactions also assert the exact campaign, participant-profile, package-version, and package-acceptance head revisions observed by the final stable policy sample, so a policy race changes none of those records. Exact replay and withdrawal derive aggregate currency from the trusted immutable indication revision instead of reinterpreting historical receipts under current campaign currency. After historical withdrawals leave both aggregate amount and contributor count at zero, a newly advertised current-currency create may compare-and-set the singleton to that currency in its complete atomic transaction; a non-empty currency mismatch remains unavailable and causes no writes. Historical retries retain their stored currency after this rollover. New create and edit use current amount policy, while a new reactivation is available only when the withdrawn fields still satisfy that policy. Exact replay is resolved before fresh policy assertions, and withdrawal remains independent of current campaign and package policy. The participant service rejects weaker mutation persistence during composition and validates the closed result before returning it.
