...
- `withdraw-founder-application` with `DELETE` while the application is received and the reserved withdrawal transition remains.
...
The route obtains a participant-bound application service from an injected factory. The trusted route or mutation actor selects that factory; body fields cannot select a subject. In the hosted runtime, an eligible active non-owner profile discovers the founder link from `/participant` without loading application state. The exact founder path deliberately negotiates authentication-required for anonymous callers and non-disclosing not-found for unauthorized authenticated callers. Configured contribution choices come from private campaign setup, while publication, phase, country, profile, and unchanged-choice policy are rechecked immediately before create. A closed phase removes create without preventing an existing application from being read, edited when current choices and budget permit, or withdrawn.
...
- `create-company-investment-interest` with `POST` when company creation, current acknowledgment, and aggregate currency compatibility permit it.

An active item may expose `edit-investment-interest` with `PATCH` only before the domain revision ceiling, when its stored currency is current, the aggregate can accept that currency, and the current package acknowledgment is valid. It may expose `withdraw-investment-interest` with `DELETE` while the domain lifecycle still permits withdrawal. A withdrawn item may expose `reactivate-investment-interest` with `POST` only before the domain revision ceiling and when its stored amount and currency, aggregate compatibility, renewed acknowledgment, and deployment policy all permit it. A rejected item exposes no participant mutation action.
...
Investment-interest reads and writes are separate injected capabilities. The participant-owned reader exposes `get`, `listOwned`, and one non-disclosing aggregate-currency compatibility bit; it never returns the private aggregate snapshot. Every write requires `AtomicParticipantInvestmentInterestMutationPort` with the explicit `atomic-indication-aggregate-audit` guarantee: the new indication revision, aggregate projection, allowlisted audit transition, and idempotent retry result are committed together or none are changed. Hosted create, edit, and reactivate transactions also assert the exact campaign, participant-profile, package-version, and package-acceptance head revisions observed by the final stable policy sample, so a policy race changes none of those records. Exact replay and withdrawal derive aggregate currency from the trusted immutable indication revision instead of reinterpreting historical receipts under current campaign currency. After historical withdrawals leave both aggregate amount and contributor count at zero, a newly advertised current-currency create may compare-and-set the singleton to that currency in its complete atomic transaction. Fresh create, edit, and reactivation are suppressed and rejected before their write port when a non-empty aggregate uses another currency; the aggregate compare-and-set still rejects a race without writes. Historical retries retain their stored currency after rollover and bypass fresh currency, policy, and lifecycle gates only after matching immutable operation evidence. New create and edit use current amount policy, while a new reactivation is available only when the withdrawn fields still satisfy that policy. Withdrawal remains independent of current campaign and package policy. The participant service rejects weaker mutation persistence during composition and validates the closed result before returning it.
