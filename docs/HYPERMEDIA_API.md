...
The aggregate record committed by a successful correction also carries one closed, bounded copy of that exact command. While the corrected aggregate remains the matching current state, HTML and JSON expose only `retry-recorded-aggregate-correction` with the same fields and no newly issued operation ID. Its proof stores only an encrypted digest binding the configured owner, route, `POST`, operation ID, campaign and aggregate revisions, confirmation, amounts, and counts. After response loss or Worker restart, an exact retry reaches the immutable receipt; changed, foreign, unrelated-method, and reused proofs fail before another durable claim or correction transaction. A later aggregate write removes the terminal command and therefore the retry action.
