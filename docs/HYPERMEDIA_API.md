...
`GET /owner/founder-applications/{review-id}` resolves exactly one opaque current application without a collection scan. It accepts no query fields. A malformed identifier returns generic `400`, an absent identifier returns generic `404`, and corrupt or unavailable persistence returns generic `503`; rejected input and backend detail are never reflected. A valid response includes the current founder fields and bounded transition history, but no applicant subject, storage key, credential, or owner identity.

The detail document always links to itself and the owner workspace. It links to the collection only when that independently composed capability is available; detail-only composition omits the unavailable transition from both JSON and HTML. HTML renders the same current state, private fields, history, and available navigation as version `0.1` JSON. Both representations use the same configured-owner authorization and verified persistent projection, are private and non-cacheable, and remain stable after Worker reconstruction. Anonymous callers receive `401` and authenticated non-owners receive generic `404` before any founder-application detail read.
