...
- exact HTTPS issuer;
- optional exact HTTPS server transport origin, defaulting to the issuer;
...
- a validated finite software deadline for each provider fetch and streamed response;
- the exact canonical application origin used by the owner CSRF session.

`APP_BASE_URL`, `AITTADB_OAUTH_ISSUER`, optional `AITTADB_OAUTH_TRANSPORT_ORIGIN`, `AITTADB_OAUTH_CLIENT_ID`, `AITTADB_OAUTH_CALLBACK_URI`, and `AITTADB_OAUTH_STORAGE_SCOPES` are ordinary per-instance runtime values. `AITTADB_OAUTH_CLIENT_SECRET`, `AITTADB_OAUTH_TRANSACTION_KEY`, and `AITTADB_OAUTH_CSRF_KEY` are hosted secrets. The ignored active Sites binding names the dedicated D1 capability as `OAUTH_PROOF_DB`; the inert tracked hosting example remains null. Every required OAuth value must be configured together.

The optional transport origin exists for Sites deployments whose custom issuer hostname cannot be reached from another Site's Worker. It changes only the backend network origin for the fixed discovery, token, and introspection paths. The authorization URL, discovery contract, callback binding, token issuer, and client audience remain bound to the logical issuer. The transport origin is never derived from a request, sent to the browser, accepted from campaign content, or supplied by reusable source.
...
Backend requests use Worker-native `fetch(url, init)`, include no cookie header, and handle redirects manually so credentials cannot follow a redirect. Token and introspection requests use HTTP Basic client authentication and bounded form bodies. Provider fetches receive an abort signal and have a finite wait ceiling; response streams have independent elapsed-time, byte, and chunk-count ceilings. Rejected status, media, declared-size, streamed-size, and timeout responses are cancelled without awaiting a potentially stuck source. Discovery, token, and introspection require exact HTTP `200`; every other status fails before body parsing. JSON response Content-Type is parsed as one media type whose case-insensitive type and subtype are exactly `application/json`; unique valid parameters are allowed, while lookalike subtypes, combined values, duplicate parameters, and malformed syntax fail closed. Response bodies are byte bounded, field allowlisted, and strictly parsed. The token must contain exactly the requested scopes and no refresh or ID token. Introspection must report an active access token with the exact issuer and client audience, an unexpired bounded lifetime, bounded integer `iat` and `nbf`, canonical UUIDv4 AittaDB subject and token ID, and exactly the requested storage scopes.

Discovery availability runs only after the route has authenticated the configured owner. An unavailable probe may emit at most one fixed `investor_app.oauth.discovery.*` operator event from the closed taxonomy: fetch, redirect, authorization, not-found, rate-limit, server, other status, content type, declared size, body availability, streamed body size, encoding, JSON, document shape, protocol contract, or internal boundary. The event contains no URL, query, configured value, identity, response content, exception, or cause. Anonymous and non-owner requests do not run discovery, and neither HTML nor hypermedia returns the private failure phase.

Owner connection and callback representations always use canonical query-free resource URLs, including rejected requests, and never reflect an incoming query string. Private responses use `no-store`, a no-referrer policy, MIME sniffing and framing defenses, and a restrictive content security policy. The authorization redirect necessarily transports OAuth state and the PKCE challenge to the configured authorization endpoint; they are not included in application documents, result metadata, logs, or persistence.
...
The selected [acceptance AittaDB service](https://test.aittadb.com/) advertises the required Authorization Code flow, S256 PKCE, confidential client authentication, introspection, and storage scopes. One confidential acceptance client is registered with the exact callback and the read-only proof scope, and its secret is installed only in the Investor acceptance runtime. Bounded hosted diagnostics established that requests to the custom issuer hostname receive a server-class edge failure before the AittaDB Worker executes; the instance's separately configured built-in Sites hostname is therefore used only as the backend transport origin. The public production [AittaDB service](https://aittadb.com/) remains separate and was not changed for acceptance testing.

No successful hosted callback is claimed yet.

## Remaining Live Proof

Local source validation cannot establish network reachability from a deployed Sites Worker to the configured acceptance transport or prove the provider callback. The remaining evidence is one successful owner-authorized hosted flow against the exact configured logical issuer and transport, with operator logs checked for fixed phase events only.
...
4. Record the hosted evidence on the integrating branch, then and only then close `BACKLOG-019` through a newly scheduled PLAN task.
