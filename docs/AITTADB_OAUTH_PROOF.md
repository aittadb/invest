...
Discovery availability runs only after the route has authenticated the configured owner. An unavailable probe may emit at most one fixed `investor_app.oauth.discovery.*` operator event identifying a bounded failure phase such as transport, status class, content type, body bound, decoding, document shape, or protocol contract. The event contains no URL, query, configured value, identity, response content, exception, or cause. Anonymous and non-owner requests do not run discovery, and neither HTML nor hypermedia returns the private failure phase.
