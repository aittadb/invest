...
- `GET /owner/aittadb-connection` returns an owner-only HTML workspace or version `0.1` hypermedia resource. It probes discovery and advertises `verify-aittadb-connection` only when the configured issuer exposes the required protocol.
...
- the shared owner-session mutation guard and CSRF provider.
...
3. Install the client secret and AES-GCM cookie key in hosted secret storage, and inject durable transaction-claim and result-sink adapters.
