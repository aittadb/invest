...
The owner reconciliation URI negotiates the same preview and preview-bound correction through hypermedia JSON or an HTML confirmation form. The shared browser mutation guard verifies trusted owner subject, exact origin, one-use expiring CSRF proof, bounded input, and method before the route maps the closed action fields. A stale preview fails before correction, retries reuse the original audit evidence, and no partial aggregate or audit write is possible through this capability. Public reads still pass through the closed aggregate sanitizer and cannot return indication identity or private contributing counts. Hosted composition reads the current campaign's amount configuration, creates a configured-owner-bound correction capability over the credential-closed AittaDB adapter, and advertises the owner link only when that complete capability is available. Synthetic hosted tests prove correction, audit atomicity, stale and storage failures, proof consumption, non-disclosure, and restart persistence; live acceptance remains a separate deployment proof.
