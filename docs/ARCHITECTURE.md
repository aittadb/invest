...
`services/participant-access.ts` combines one subject-bound profile, current package, and current-acceptance result into the narrow authorization state consumed by the Worker. Each profile and package sample is validated, bounded, and detached before another asynchronous read. The service samples both sides of the acceptance read, permits one bounded retry for a concurrent profile or package change, and fails closed if movement continues. A missing profile returns before package or acknowledgment storage is read.
