...
The persistent participant coordinator allows at most four active owned indications across personal and company records. Before a new command is staged, it materializes the bounded participant index and verifies every current projection; a legacy state with more than four active records or any missing, crossed, or malformed projection fails closed. Create and reactivation conflict at the limit. Participant creation, withdrawal, and reactivation also compare-and-set the same participant-index revision, so concurrent activations cannot cross the bound. Withdrawn and rejected records remain in the 100-item historical index without consuming an active slot. Four is a persistence safety invariant, not campaign configuration: it keeps the maximum account-deletion withdrawal set within the storage protocol's 25-mutation atomic boundary.
