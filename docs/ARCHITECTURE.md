...
Reads reconstruct the bounded transition ancestry from exact data-only records, verify every field reference, chunk topology, byte count, content hash, operation fingerprint, storage revision, and required active lease, then compare the reconstruction with current metadata. Retry-stable operations return their original revision even after later transitions or deployment amount-policy changes; changed retries and stale writes fail without mutating indication, aggregate, or founder state. Exported record, transaction, mutation, materialization-read, and delayed-replay-read ceilings are enforced and observed in the reusable repository contract.
