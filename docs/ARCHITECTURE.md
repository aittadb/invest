...
First persistent registration now composes authoritative empty ownership initialization into the same transaction. `TASK-159` still supplies the backend-only complete-inventory migration for participants created before that contract. Hosted investment composition must remain unavailable until the migration prerequisite is complete for the relevant legacy population.
