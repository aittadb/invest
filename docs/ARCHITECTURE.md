...
A deletion request persists only its existing request state and exposes a separate application-level withdrawal intent; the repository does not import or mutate indication or founder storage. Anonymous, owner-without-profile, foreign, denied, and missing records retain the same non-disclosing shapes. Hosted participant authorization and route composition remain separate work in `TASK-090`, `TASK-091`, and `TASK-092`; this repository does not install itself in the Worker.
