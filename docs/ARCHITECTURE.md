...
`services/legacy-investment-ownership-migration.ts` exposes one narrow backend migration over that initialization primitive. It parses a complete, sorted manifest of at most 100 well-formed Unicode subjects and at most 100 ownership entries per subject, requires globally unique indication and retry-stable operation IDs, rejects static over-capacity before any read, processes no implicit or discovered subject, and returns only counters. `scripts/migrate-legacy-investment-ownership.ts` closes the AittaDB credential inside the service-token provider and adapter, accepts only explicit `--apply --manifest`, rejects symbolic links and duplicate JSON members, and parses only a bounded byte-stable regular-file snapshot whose identity metadata remains unchanged across two reads. The command uses no browser route, collection listing, ordinary-request absence fallback, Sites binding, or checkpoint store; immutable root, index, and witness evidence provides exact restart and partial-resume behavior without an operation receipt.

The capacity boundary is not production-ready on its own. `TASK-158` composes authoritative empty ownership initialization into first registration. Hosted investment composition must remain unavailable until that provisioning exists and every applicable legacy participant has completed the reviewed migration.
