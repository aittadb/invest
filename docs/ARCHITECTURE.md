...
Hosted Worker composition installs that route group for eligible investor and combined participants from the current subject-bound request scope. The same scope owns the persistent indication reader and mutation port, so indication and policy reads cannot bypass the participant request budget. It binds the trusted campaign amount configuration, participant profile, current package and latest stored acknowledgment, and hosted mutation session without exposing any of them to the browser. One request-shared policy loader brackets package and acknowledgment reads with matching campaign and participant revisions. The scope retains only the latest observed revisions of the campaign, participant-profile, package-version, and package-acceptance heads. Create, edit, and reactivate then include four non-mutating checks for those exact heads in the same transaction as indication, aggregate, audit, index, and receipt writes. A head change after the final stable sample therefore rolls back the complete mutation. Exact operation replay is resolved from its immutable receipt before fresh policy assertions, while withdrawal remains available without policy-head checks. Both paths validate aggregate evidence against the immutable indication revision's currency, so a later campaign-currency change cannot reinterpret or strand historical operations. Once historical withdrawals leave the aggregate at exactly zero amount and zero contributors, a fresh current-currency create may roll the singleton currency through the same compare-and-set transaction; any non-empty currency mismatch fails closed. Historical receipts retain their own currency across that rollover, and fresh reactivation remains gated by current currency and amount rules. The exact hosted URI remains an intentional negotiated 401 or non-disclosing 404 surface for anonymous, owner, ineligible, and foreign callers, while participant-home navigation is advertised only when the complete route capability is installed.
