...
The scope owns one 574-record participant-private read budget. A maximum valid authorization consumes at most 551 reads: 511 unique immutable package records plus two outer attempts, each with six profile reads, two package-head reads, and three accepted gate attempts of four mutable reads. The six profile reads include current, matching latest history, and immutable registration revision 1 for each of the two profile samples. The selected route may then consume at most 23 more reads. That ceiling covers profile current-state validation, two historical samples, one failed mutation attempt, recovery state and history, the retry, and the final projection; package GET needs one and acknowledgment GET needs at most four. Read 575 fails before reaching the credential-bound adapter. Public campaign reads and adapter transactions are separate operations, but every profile, package, gate, acceptance-head, acceptance-record, notice-anchor, and participant-route read shares this one request counter.
...
Hosted Worker composition installs that route group for eligible investor and combined participants from the current subject-bound request scope. It binds the trusted campaign amount configuration, participant profile, current package and latest stored acknowledgment, persistent indication repository, and hosted mutation session without exposing any of them to the browser. Creation and reactivation re-read publication, phase, country, participant status, and campaign revision before use; a campaign change during one request fails closed. Package state is read around the latest acceptance so a concurrent package change cannot authorize a stale acknowledgment. The exact hosted URI remains an intentional negotiated 401 or non-disclosing 404 surface for anonymous, owner, ineligible, and foreign callers, while participant-home navigation is advertised only when the complete route capability is installed.
