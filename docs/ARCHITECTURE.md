...
`services/legacy-investment-ownership-migration.ts` exposes one narrow backend migration over that initialization primitive. It parses a complete, sorted, duplicate-free manifest of at most 100 subjects and at most 100 ownership entries per subject, requires unique retry-stable operation IDs, rejects static over-capacity before any read, processes no implicit or discovered subject, and returns only counters. `scripts/migrate-legacy-investment-ownership.ts` closes the AittaDB credential inside the service-token provider and adapter, accepts only explicit `--apply --manifest`, and reads one bounded ignored file with growth detection. The command uses no browser route, collection listing, ordinary-request absence fallback, Sites binding, or checkpoint store; exact per-subject operation replay provides restart and partial-resume behavior.
