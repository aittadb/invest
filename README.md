...
The manifest contains `schemaVersion: 1` and at most 100 sorted `participants`. Each participant supplies its well-formed subject, one globally unique retry-stable operation ID, and the complete sorted set of at most 100 indication IDs with current revision and `active`, `withdrawn`, or `rejected` status; indication IDs are unique across the whole manifest. At most four may be active. The command reads a stable non-symlink snapshot, never lists participants, has no browser route, does not use the Sites runtime credential, and prints only counters. Exact reruns resume safely; changed, duplicate-key, incomplete, crossed, corrupt, oversized, or over-capacity input fails closed. See [docs/HOSTED_AITTADB_RUNTIME.md](docs/HOSTED_AITTADB_RUNTIME.md#legacy-investment-ownership-migration) for the operator contract. Do not run it against a live namespace without explicit approval and an independently reviewed authoritative inventory.
