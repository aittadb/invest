...
- Cross-repository mutations stage behind a request-local `StorageAdapter` overlay and reach durable storage only through one final bounded transaction. The overlay rejects duplicate keys, stale revisions, unsafe same-collection list/write mixtures, oversized requests, and mismatched ordered evidence; response-loss retries reuse the same operation ID and fixed failures discard adapter causes.
