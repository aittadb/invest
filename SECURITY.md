...
- Owner indication detail reads authorize the configured owner before opaque-ID resolution, resolve exactly one authenticated current-record key without listing, and reconstruct only bounded verified history. Malformed, missing, crossed, corrupt, anonymous, and non-owner reads use fixed non-disclosing outcomes.
